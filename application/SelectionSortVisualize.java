...
public class SelectionSortVisualize extends AbstractSort {

	private static final Color MININDX_COLOR = Color.ORANGE;
	  private static final Color NEW_MININDX_COLOR = Color.LIGHTGREEN;

	  private ParallelTransition colorCNode(DataNode[] arr, int a, int b, Color colorA, Color colorB) {
	      ...
	      for (int j = i + 1; j < arr.length; j++) {
	        ...
	        if (arr[j].getValue() < arr[minIndx].getValue()) {
	          if (minIndx == i) {
	            ...
	            transitions.add(colorCNode(arr, minIndx, j, START_COLOR, NEW_MININDX_COLOR));
	          }
	          minIndx = j;
	        } else {
